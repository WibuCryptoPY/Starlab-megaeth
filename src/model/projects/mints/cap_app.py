...
STORAGE_SCAN_CONTRACT = Web3.to_checksum_address("0xe9b6e75c243b6100ffcb1c66e8f78f96feea727f")
...
class CapApp:
    ...
    async def mint_cUSD(self):
        try:
            ...
            payload = f"0x40c10f19000000000000000000000000{wallet_address_no_prefix}00000000000000000000000000000000000000000000003635c9adc5dea00000"

            # Prepare basic transaction
            tx = {
                "from": self.wallet.address,
                "to": STORAGE_SCAN_CONTRACT,
                "data": payload,
                "value": 0,
            }
            ...
            if receipt["status"] == 1:
                logger.success(
                    f"{self.account_index} | CapApp cUSD minted successfully! TX: {EXPLORER_URL_MEGAETH}{tx_hex}"
                )
                return True
            else:
                raise Exception(
            ...
            logger.error(
                f"{self.account_index} | Error minting CapApp cUSD: {e}. Waiting {random_pause} seconds..."
            )
